      PROGRAM RD
C reads cos blocked files on unix system and summarizes file contents.
C modified for use on PC with DVF (Digital Visual Fortran)
C
      PARAMETER(IUN=1,MSGUN=6,LMWD=32,IRW=1,LWK=4200)
      PARAMETER(MXLBLK=20000)
      CHARACTER NAME*64
      PARAMETER (WD2FT6=LMWD/(8.*6250.*12.), EOR6=.3/12., EOF6=4.5/12.,
     +           WD2FT1=LMWD/(8.*1600.*12.), EOR1=.6/12., EOF1=6.0/12.)
C		 WD2FT_ = Multiplier to convert # words to feet of tape
C			  at 6250 or 1600 bpi
C		 EOR_	= Length of EOR gap at _ density in feet
C		 EOF_	= Length of EOF gap at _ density in feet
      DIMENSION IWK(LWK),IBLK(MXLBLK)
C
      WRITE(6,*)' ENTER FILE NAME TO READ '
      READ(5,'(A)')NAME
      PRINT 1000
 1000 FORMAT(' ' ,/,'  EOF STATUS BLOCKS  WORDS MIN RECL',
     +	     ' MAX RECL TOT BLKS TOT WRDS FT@1600 FT@6250')
    ...
    5 CALL CBFRD (IUN,IBLK,MXLBLK,LBLK,NUB,IST,IWK)
      ...
      NCE=0
      IF(LBLK .GT. MX) MX=LBLK
      IF(LBLK .LT. MN) MN=LBLK
      NWDS = NWDS+LBLK
      NR   = NR+1
      RWDS = REAL (LBLK)
      FT1600 = FT1600 + RWDS*WD2FT1 + EOR1
      FT6250 = FT6250 + RWDS*WD2FT6 + EOR6
C>>>>	   Remove comment to print 1st 10 block's lengths:
C     IF(NR .LT. 11)PRINT 1001,NF,IST,NR,LBLK
      GO TO 5
   12 CONTINUE
      NF  = NF+1
      NWT = NWT+NWDS
...
C          IST=1 or IST=3
	...
	FT6250 = FT6250 + EOF6
      ENDIF
      PRINT 1001,NF,IST,NR,NWDS,MN,MX,NRT,NWT,FT1600,FT6250
 1001 FORMAT(I5,3I7,4I9,2F8.1)
C>>>>	   Remove comment to add space after each eof summary prt
C     WRITE(6,'('' '')')
      NCE=NCE+1
      IF(NCE.LE.1) GO TO 2
      END
...
c       WRITE (IUN,REC=IWK(8)) (IWK(I),I=IWK(13),IWK(13)+IWK(14)-1)
...
C          Zero out the remainder of the Cray block, then write it
 ...
 1630 IWK(I) = 0
      IWK(8) = IWK(8) + 1
Cbyte      non-byte reversed version:
C     WRITE (IUN,REC=IWK(8)) (IWK(I),I=IWK(13),IENB)
Cbyte      byte reversed version (works on PC's and DEC's)
      CALL SWAP4 (IWK(IWK(13)),IWK(IWK(15)),4096)
      WRITE (IUN,REC=IWK(8)) (IWK(I),I=IWK(15),IWK(15)+IWK(14)-1)
Cbyte      end
...
Cbyte      non-byte reversed version
c   5 IPNB  = IWK(13) - 1
c     IPNBF = IWK(15) + IWK(10) - 1
c     DO 10 I=1,NMWD
c     IPNB  = IPNB  + 1
c     IPNBF = IPNBF + 1
c  10 IWK(IPNB) = IWK(IPNBF)
Cbyte      end
