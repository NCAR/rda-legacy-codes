      program  restore
C
c this code is a test version and may still have bugs
c
      dimension iwk(2068),ni(8000),no(8000),nh(32000)
      dimension jwk(2068)
      character vsn*128,vsno*128,noc*320000
      equivalence(no,noc)
c put binary data back in contiguous bits after read on 7trk (nconv=1)
c    or
c translate 6-bit bcd to ASCII (nconv=0 (linefeed) or nconv=2 (cosb))
c
      nconv=0
c
c set ipre=1 if 7 trk tape read has prefixed status and length
c       otherwise ipre=0
      ipre=1
      nr=0
      ner=0
      nce=0
      tcnt=0.
      write(*,*) ' Enter file to translate '
      read (*,'(a)')vsn
      lc=index(vsn,' ')
      call cbfopn(11,vsn,6,32,1,iwk,2068)
      if(nconv.ne.0) then
	vsno=vsn(1:lc-1)//'-cosb'
	call cbfopn(21,vsno,6,32,1,iwk,2068)
      else
	vsno=vsn(1:lc-1)//'-asc'
	open(21,file=vsno,form='formatted')
      endif
   10 call cbfrd(11,ni,8000,lth,nub,ist,iwk)
      if(ist.ne.0) go to 90
      nce=0
      nr=nr+1
      if(ipre.eq.0) then
	lthb=(32*lth-nub)/8
	call gbytes(ni,nh,0,8,0,lthb)
      else
	istat=ni(1)
	lthb=ni(2)
	if(istat.ne.0) then
c         write(*,1002)nr,istat,lthb
 1002     format(' Bad Status, rec,stat,lth ',4i8)
	  ner=ner+1
	endif
	call gbytes(ni,nh,64,8,0,lthb)
      endif
      if(nconv.eq.1) then
	call sbytes(no,nh,0,6,0,lthb)
      else
	call bcd2as(nh,nh,lthb)
      endif
c write out as linefeed delimited or cbfwrite
      if(nconv.eq.0) then
	write(21,'(a)')noc(1:lthb)
      else
	xbits=lthb*6
	nbits=xbits
	nwdw=(xbits+31.1)/32.
	nubw=32*nwdw-nbits
	call cbfwr(21,noc,8000,nwdw,nubw,jstat,jwk)
      endif
      go to 10
   90 continue
      nce=nce+1
      if(nce.le.1) go to 10
      end
...
C          The "byte" is to be stored in middle of word, so shift left.
    ...
    6 CONTINUE
      ...
      RETURN                                                                    
      END                                                                       
      SUBROUTINE BCD2as(IN,nOUT,N)
      DIMENSION IN(1),nout(1)
      character ntab(64)*1
      DATA nTAB/':','1','2','3','4','5','6','7','8','9','0','?',
     2         '?','?','%','[',' ','/','S','T','U','V','W','X',
     3         'Y','Z',']','\'','(','?','?','^','-','J','K','L',
     4         'M','N','O','P','Q','R','?','$','*','?','?','>',
     5         '+','A','B','C','D','E','F','G','H','I','<','.',
     6         ')','?','?',';'/
      DO 20 I=1,N
      II=IN(I)+1
      IF(II .GT. 64 .OR. II .LT. 1) II=1
      NOUT(I)=ichar(nTAB(II))
   20 CONTINUE
      RETURN
      END
