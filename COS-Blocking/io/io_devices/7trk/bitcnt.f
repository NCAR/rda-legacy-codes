      program  restore
      dimension iwk(2068),ni(8000),nh(32000)
      dimension jwk(2068),ibits(8),cnt(8),pct(8)
      character vsn*8
c put binary data back in contiguous bits after read on 7trk
...
c       otherwise ipre=0
	...
	cnt(i)=0.
      enddo
      write(*,*) ' Enter file to scan '
      read (*,'(a)')vsn
      call cbfopn(11,vsn,6,32,1,iwk,2068)
   10 call cbfrd(11,ni,1000,lth,nub,ist,iwk)
...
c         write(*,1002)nr,istat,lthb
 1002     format(' Bad Status, rec,stat,lth ',4i8)
	...
	call gbytes(ni,nh,64,8,0,lthb)
      ...
      tcnt=tcnt+lthb
      do i=1,lthb
	call gbytes(nh(i),ibits,24,1,0,8)
	do ii=1,8
	  if(ibits(ii).eq.1) cnt(ii)=cnt(ii)+1.
	enddo
      enddo
      write(*,*)'Record ',nr,'  Length ',lthb
      do ii=1,lthb,40
	ie=ii+39
	if(ie.gt.lthb) ie=lthb
	write(*,'(i6,"-",40z2)')ii,(nh(i),i=ii,ie)
      enddo
      go to 10
   90 continue
      nce=nce+1
      ...
      do i=1,8
	pct(i)=100.*cnt(i)/tcnt
      enddo
      write(*,1001)vsn,nr,ner,tcnt,pct
 1001 format(a,' recs read, errs ',2i8,' bit totals ',f12,8f5)
      write(*,1003)vsn,nr,ner,tcnt,cnt
 1003 format(a,' recs read, errs ',2i8,' bit totals ',f12,8f8.0)
      end
