      program t2d
      parameter (ndim=16384,maxl=ndim,maxb=2+(maxl+4)/4)
      character nbf*(ndim),flnm*64,flnmx*64
      character aden*1,apar*1,acom*32
      dimension iwk(2100),nbfb(maxb),icom(32)
      dimension idmy(3),ihms(3)
      logical label
      equivalence (nbf,nbfb(3))
      label=.FALSE.
c
c set units for std output and tape error diagnostics
      io=7
      ie=9
c set output log and tape unit
      lu=8
      iun=0
c
      nce=0
...
c set no file limit
      nrlim=99999999
c set all files into one cosb file
      multi=0
      nf=1
c
      lamin=ndim
      lamax=0
      read(*,'(a)')flnm
c     flnmx='/scratch/7trk/'//flnm(1:ix)
      read(*,'(a)')flnmx
      ix=index(flnm,' ')
      iy=index(flnm,',')
      if(iy.gt.0 .and. iy.lt.ix) ix=iy
      aden=flnm(ix+1:ix+1)
      apar=flnm(ix+3:ix+3)
c
      call open7(iun,acom,icom,aden,apar,nbf,maxl,ie,istat)
      if(istat.eq.0) then
	write(io,1007)flnm(1:ix),aden,apar,flnmx
 1007   format(' opened ',a,' den = ',a,'  parity = ',a,
     2 '   writing to ',a)
      elseif(istat.eq.1) then
	write(io,1008)flnm(1:ix),aden,apar,flnmx
 1008   format(' problem open ',a,' den = ',a,'  parity = ',a,
     2 '   writing to ',a)
      else
	write(io,1002)istat
 1002   format(' Tape device not opened, status = ',z9)
	go to 99
      endif
      call CBFOPN (21,flnmx,7,32,2,IWK,2100)
   ...
   10 continue
      ...
      if(istat.eq.1 .or. istat.eq.3) go to 80
      nce=0
      nr=nr+1
      nrt=nrt+1
c
      if(istat.ne.0) then
	write(io,1004)nr,istat,lth
 1004   format(' unrecovered read error- nr,istat,lth ',i8,i4,i6)
	nerr=nerr+1
	ncerr=ncerr+1
	net=net+1
	nebt=nebt+lth
	if(ncerr.gt.2400) go to 95
c        skip zero length records
	if(lth.le.0) go to 10
      else
	ncerr=0
      endif
      if(lth.gt.lmax) lmax=lth
      if(lth.lt.lmin) lmin=lth
      if(lth.gt.lamax) lamax=lth
      if(lth.lt.lamin) lamin=lth
      nbt=nbt+lth
      ltho=(lth+3.1)/4.
      nub=32*ltho - 8*lth
      ltho=ltho+2
      nbfb(2)=lth
      nbfb(1)=istat
      CALL CBFWR (21,nbfb,maxb,ltho,NUB,istat,IWK)
      if(mod(nr,10000).eq.1) then
	write(io,1001)nf,nr,lth,istat,ltho,nub
 1001   format(' Copying-file,rec,lth,stat ',i6,2i12,i3,
     2   '  words,nub ',i4,i3)
      endif
      go to 10
   80 continue
c prevent writing empty files
      if(nce.gt.0) go to 88
      call cbfeof(21,iwk)
      if(multi.eq.1) then
	call cbfcls(21,iwk)
	nfx=nf+10000
   ...
   88 continue
      write(io,1000)nf,nr,istat,lmin,lmax,nrt,nbt
 1000 format(' file,rec,status,min,max,totrecs, totbytes',
     2 i5,i6,i4,2i8,i8,i15)
c test for number of consec empty files
c  give less weight to pseudo file marks
      if(istat.eq.1) then
	nce=nce+2
      else
	nce=nce+1
      ...
      nf=nf+1
c
      if(nce.lt.4 .and. nf.lt.nrlim)  go to 9
      call cbfcls(21,iwk)
      go to 99
   95 continue
      write(io,1006)nr,istat,flnmx(1:ix+5)
...
c output log record
      ...
      call idate(idmy)
      call itime(ihms)
      write(lu,1009)flnm(1:ix),nf,nrt,nbt,net,nebt,lamin,lamax,
     2 (idmy(i),i=3,1,-1),(ihms(j),j=1,2)
      write(io,1009)flnm(1:ix),nf,nrt,nbt,net,nebt,lamin,lamax,
     2 (idmy(i),i=3,1,-1),(ihms(j),j=1,2)
 1009 format(' LOG- ',a8,i5,i8,i10,i8,i10,2i6,2x,i4,2i2,1x,i2,':',i2)
      call rewind7(iun,acom,icom,istat)
...
Cold  NFWI = MIN0 (512-IWK(6)/NWPC,NWDC)
      IF (IWK(5) .GT. 0) THEN
