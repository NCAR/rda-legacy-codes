...
c
      ...
      read(*,*)nrlim
      if(nrlim.le.0) nrlim=99999999
      write(*,*)' New UNIX file for each input file? (0=no,1=yes) '
      read(*,*)multi
      nf=0
c
      write(*,*)' Enter device to read (/dev/nrmt0) '
      read(*,'(a)')dev
      iy=index(dev,' ')-1
      ist=tclose(iun)
 ...
 1003 format(' Tape device opened, status = ',3z9,1x,a)
      ...
      flnmx(ix:ix)='.'
      call CBFOPN (21,flnmx,6,32,2,IWK,2100)
    9 nr=0
      nerr=0
      ncerr=0
      lmax=0
...
c       ist=-ist
      ...
      nub=32*ltho - 8*ist
c     nub=0
      CALL CBFWR (21,nbf,8192,ltho,NUB,istat,IWK)
      if(mod(nr,1000).lt.10) then
c       write(*,1001)nf,nr,ist,istat,nce,ltho,nub
	write(7,1001)nf,nr,ist,istat,nce,ltho,nub
 1001   format(' Copying-file,rec,lth,stat ',i6,3i12,i3,
     2   '  words,nub ',i4,i3)
      endif
      go to 10
   80 continue
c prevent writing empty files
      if(nce.gt.0) go to 78
      call cbfeof(21,iwk)
      nf=nf+1
      if(multi.eq.1) then
	call cbfcls(21,iwk)
