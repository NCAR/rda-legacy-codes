      program read
c
c sample driver code for rptin on IBM PC with Digital Visual Fortran
...
c       The buffer must be at least big enough to contain
c       16192 bytes plus 26 words(integer words on the machine
c       the code is executing on)
c
      parameter (nwsz=32,ndim=26+(16192*8+nwsz-1)/nwsz,ldim=1000)
      dimension nbuf(ndim),nb(ldim)
      nr=0
      nbuf(1)=0
      nbuf(2)=0
    5 call rptinx(11,nbuf,nb,lth,0,ldim,ist)
      if(ist.ne.0) go to 9
      nr=nr+1
      write(*,1001)nr,lth,nb(1),nb(lth-1),nb(lth)
 1001 format(1x,2i6,1x,3z9)
      go to 5
    9 continue
      write(*,1002)nr
...
c          Use character*1 instead of logical*1 when compling on a Cray
	do 10 i=1,nn,4
	ih=in(i)
	io(i)=in(i+3)
	io(i+3)=ih
